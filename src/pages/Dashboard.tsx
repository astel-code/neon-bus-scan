...
import { Users, UserCheck, AlertTriangle, Download, RotateCcw, Phone } from "lucide-react";
import { getDashboardStats, exportCSV, resetTodayScans, resetStudentDatabase } from "@/lib/storage";
...
const Dashboard = () => {
  ...
  // Reset student DB to latest defaults on mount
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <AnimatedCounter value={stats.presentCount} label="Present Students" icon={UserCheck} color="text-neon-green" delay={150} />
        <AnimatedCounter value={stats.missingCount} label="Missing Students" icon={AlertTriangle} color="text-neon-orange" delay={300} />
      </div>

      {/* Present Students */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.5 }}
        className="glass-card gradient-border mb-8 overflow-hidden">
        <div className="flex items-center gap-3 border-b border-border p-5">
          <UserCheck className="h-5 w-5 text-neon-green" />
          <h2 className="text-lg font-bold text-foreground">Present Students</h2>
          <span className="ml-auto rounded-full bg-neon-green/20 px-3 py-0.5 text-xs font-semibold text-neon-green">{stats.presentCount}</span>
        </div>
        {stats.presentCount === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {stats.presentStudents.map((student, i) => (
                  <motion.tr key={student.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.4 + i * 0.08 }} className="border-b border-border/50 transition-colors hover:bg-muted/30">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}
        ...
        <div className="flex items-center gap-3 border-b border-border p-5">
          ...
          <span className="ml-auto rounded-full bg-neon-orange/20 px-3 py-0.5 text-xs font-semibold text-neon-orange">{stats.missingCount}</span>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {stats.missingStudents.map((student, i) => (
                  <motion.tr key={student.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.5 + i * 0.08 }} className="border-b border-border/50 transition-colors hover:bg-muted/30">
                    <td className="px-5 py-4 font-medium">{student.name}</td>
                    <td className="mono px-5 py-4 text-muted-foreground">{student.registerNumber}</td>
                    <td className="hidden px-5 py-4 text-muted-foreground sm:table-cell">{student.department}</td>
